...
| v1.6 | 功能介面一致性 — Logo 設定新增「啟用」Toggle | 2026-02-01 |
...
### v1.6 — 功能介面一致性改進 (2026-02-01)

**改進內容：**
- 所有功能區塊統一加入「啟用」Toggle 開關，達到介面一致性
- Logo 設定是唯一缺少啟用開關的區塊，現已補齊

**四大區塊啟用開關一覽：**

| 區塊 | Toggle 位置 | 顏色 | 關閉行為 |
|------|------------|------|----------|
| Logo 設定 | 標題列右側 | `blue-500` | 隱藏設定、不嵌入 Logo |
| 浮水印 | 標題列右側 | `cyan-500` | 隱藏設定、不繪製浮水印 |
| 頁首/頁尾 | 各列右側 | `purple/emerald-500` | 隱藏設定、不繪製頁首/頁尾 |
| PDF 安全性 | 標題列右側 | `blue-500` | 隱藏設定、不加密 |

**修改檔案：**
- `utils/settingsStorage.ts`：`UserSettings` 新增 `logoEnabled: boolean`，預設 `true`，含驗證邏輯
- `components/Settings/LogoSettings.tsx`：新增 `logoEnabled` / `onEnabledChange` props，條件渲染內容區塊
- `App.tsx`：新增 `logoEnabled` state + localStorage 持久化，下載時根據開關決定是否傳入 logo bytes
